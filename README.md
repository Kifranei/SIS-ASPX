...
本项目是一个面向高校教学管理场景的综合信息系统，覆盖管理员、教师、学生三类角色的常见业务流程。系统提供课程管理、排课、选课、成绩录入、考试管理、课表查看、密码管理等功能，并兼顾桌面端与移动端的使用体验。
...
- **必修课学生管理**：在课表页右上角新增“课程学生管理”入口，仅显示本人必修课，并支持添加/移除学生。
...
- 重建了 `db/sql.sql` 初始化脚本，脚本内容与当前数据库结构保持一致，并将测试账号密码改为哈希存储，避免再向数据库写入明文密码。
