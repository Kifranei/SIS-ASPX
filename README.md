# 学生教学管理一体化信息系统（Web Forms 迁移版）

这是一个为毕业设计准备的教学管理系统。当前仓库已从原 MVC 5 版本迁移为 **ASP.NET Web Forms（.aspx）主入口**，满足基于课程所学 Web Forms 技术进行答辩与演示的要求。

本次迁移优先保证“可重新跑通”与“角色主流程可用”：已提供 Web Forms 登录页、管理员首页与学生列表、教师首页、学生首页；原 MVC/Controller 代码保留在仓库中，作为后续继续迁移其余页面的参考实现。

## 迁移完成情况（2026-03）

- 已完成登录/注销与三角色（管理员、教师、学生）主流程页面的 Web Forms 化，主要业务页面可直接用 `.aspx` 访问。
- 已完成对外 URL 整理：地址栏默认不再暴露 `/WebForms`，旧地址会自动跳转到新地址。
- 已恢复并统一主要前端体验：响应式侧边栏（含汉堡菜单）、暗色模式、课表按周/总览切换与导出图片。
- 已修复关键数据操作稳定性：删除学生/课程时的外键级联冲突处理，避免常见 `DbUpdateException`。
- 已补齐“必修课由教师/管理员分配学生”能力：
  - 管理员：在“编辑课程信息”页可对必修课添加/移除学生。
  - 教师：在课表页右上角新增“课程学生管理”入口，仅显示本人必修课，并支持添加/移除学生。
## 主要功能
...
系统支持暗色模式开关。用户在登录后可在页面头部切换深浅色显示，切换状态会保存在浏览器本地，下次访问会自动沿用上次设置。
...
- **主框架**: ASP.NET Web Forms (.aspx)
- **后端**: .NET Framework 4.8.1, Entity Framework 6 (Database First), ASP.NET Web API
- **前端**: HTML5, CSS3, Bootstrap 3, jQuery
- **数据库**: Microsoft SQL Server
- **核心第三方库**: html2canvas.js (用于课表截图)，Fido2NetLib（用于实现现代 WebAuthn 通行密钥支持）
- **开发环境**: Visual Studio 2019+ , C# 7.3
...
1. 打开 Visual Studio ，选择克隆存储库 ，将 `https://github.com/Kifranei/SIS-ASPX.git`  粘贴到存储库位置的输入框中，执行克隆，随后打开项目根目录下的 `Web.config` 文件。
...
本项目的完成离不开以下 AI 的协助：Google Gemini、Authropic Claude、OpenAI ChatGPT。帮我排除了不少 bug，以及实现了一些复杂的功能（如 WebAuthn 无密码认证体系与智能排选课拦截和节假日排课处理逻辑），和处理了项目从 ASP.NET MVC 5 架构迁移到 ASP.NET Web Forms 中遇到的问题。
