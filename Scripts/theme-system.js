...
(function() {
    ...
    function bindEvents() {
        ...
        document.addEventListener('click', function (e) {
            ...
            const headerBtn = target.closest('.theme-settings-header-btn');
            if (headerBtn) {
                e.preventDefault();
                e.stopPropagation();
                toggleMenu();
            ...
            if (target.closest('.theme-menu-overlay')) {
                hideMenu();
                return;
            }
            ...
            const inMenu = !!target.closest('.theme-floating-menu');
            const menu = document.querySelector('.theme-floating-menu');
            if (menu && menu.classList.contains('show') && !inMenu) {
                hideMenu();
            }
        });
        ...
        document.addEventListener('change', function (e) {
            const rawTarget = e.target;
            const target = rawTarget && rawTarget.nodeType === 3 ? rawTarget.parentElement : rawTarget;
            if (target && target.matches && target.matches('.dark-mode-toggle input')) {
                toggleDarkMode(!!target.checked);
            }
        });

        console.log('Theme system events bound once');
    }
// 显示/隐藏菜单
    function toggleMenu() {
        const menu = document.querySelector('.theme-floating-menu');
    ...
    window.addEventListener('load', function() {
        // 如果按钮不存在，重新初始化
        setTimeout(function() {
            if (!document.querySelector('.theme-settings-header-btn') || !document.querySelector('.theme-floating-menu')) {
                console.log('Theme system entry/menu not found, reinitializing...');
                initThemeSystem();
            }
        }, 500);
    });
