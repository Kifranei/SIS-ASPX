...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        using (var db = new StudentManagementDBEntities())
        {
            ...
            if (!string.IsNullOrWhiteSpace(SearchString)) query = query.Where(s => s.StudentName.Contains(SearchString) || s.StudentID.Contains(SearchString));
...
<form method="get" class="form-inline"><div class="form-group"><label>查找学生：</label><input type="text" name="searchString" value="<%= H(SearchString) %>" class="form-control" placeholder="输入姓名或学号" /></div> <button type="submit" class="btn btn-default">搜 索</button></form>
<br />
<p><a class="btn btn-primary" href="AddStudent.aspx">添加新学生</a></p>
<div class="table-responsive">
    <table class="table table-striped table-bordered">
        <thead><tr><th>学生姓名</th><th>性别</th><th>班级</th><th>登录账号</th><th>操作</th></tr></thead>
        <tbody>
            <% if (StudentsList.Any()) { foreach (var item in StudentsList) { %>
            <tr>
            ...
            <% } } else { %><tr><td colspan="5" class="text-center text-muted">暂无学生记录。</td></tr><% } %>
        </tbody>
    </table>
</div>
